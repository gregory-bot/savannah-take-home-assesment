...
4. [Section 3 — Deployment & CI/CD](#section-3--deployment--cicd)
...
2. Run the full test suite with coverage (`pytest tests/ -v --cov=app`)
3. Block merge if any test fails

**On merge to `main`:**
1. All tests pass (required by the PR check)
2. A Render deploy hook is called via the Render API, triggering an automatic
   re-deployment of the `main` branch
